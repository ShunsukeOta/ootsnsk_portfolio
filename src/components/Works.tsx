...
export default function Works () {
  return (
    <>
      ...
      <Accordion.Root variant="subtle" collapsible defaultValue={["a"]}>
        {items.map((item, index) => (
          <Accordion.Item key={index} value={item.value} py="2"> 
            <Accordion.ItemTrigger>
              <Stack gap="0" key="subtle" flex="1">
                <Text fontSize={{base: "sm", md: "md"}} fontWeight="semibold">{item.title}</Text>
                <Text fontSize={{base: "xs", md: "sm"}} color="fg.muted">{item.date}</Text>
              </Stack>
